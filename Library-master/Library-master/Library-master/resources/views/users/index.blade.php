...
@section('content')
    ...
    <div class="container">
      <div class="mt-3 mb-3">
        <a class='btn btn-sm btn-primary' href="{{ url('users/report/download') }}">Download PDF</a>
         <a class='btn btn-sm btn-info' href="{{ url('users/report/view') }}">view PDF</a>
      </div>
        <form class="row">
        <div class="col-3">
        ...
        <table class="table mt-3 table-striped table-hover">
          ...
          <tbody>
            ...
            @foreach($users as $user)
                <tr>
                  <th scope="row">  <img style="width:65px; highth:65px;   border-radius: 50%; " class="img-fluid img-rounded img-thumbnail" src='/storage/images/{{$user->photo}}' /> </th>
                  <td>{{$user->name}}</td>
                  <td>{{$user->email}}</td>
